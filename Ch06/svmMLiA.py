...
def smoSimple(dataMatIn, classLabels, C, toler, maxIter):
    ...
    说明：
        ...
        当迭代次数小鱼最大迭代次数时（外循环）
            ...
            如果该数据向量可以被优化：
                ...
                同时优化这两个向量
                如果两个向量都不能被优化，退出内循环
        如果所有向量都没有被优化增加迭代数目，继续下一次循环
    :param dataMatIn: 数据集
    :param classLabels: 类别标签集
    :param C: 常数
    :param toler: 容错率
    :param maxIter: 最大循环数
    :return:
    ...
    while (iter < maxIter):
        ...
        for i in range(m):
            ...
            # 如果误差很大，那么就可对该数据实例所对应的alpha值进行优化
...
if __name__ == '__main__':
    ...
    #     if alphas[i] > 0.0:
    #         print("data is ==> %s, label is ==> %s" % (dataArr[i], labelArr[i]))
    #
    ws = calcWs(alphas, dataArr, labelArr)
    datMat = mat(dataArr)
    category = datMat[2] * mat(ws) + b
